...
BACKUP_BASE_URL = st.secrets.get("BACKUP_BASE_URL", "")  # no hardcoded default
...
def get_ai_model():
    ...
    candidate_models = ['gemini-2.0-flash-lite', 'gemini-1.5-flash', 'gemini-1.5-pro']
    last_error = ""
...
def get_ai_response(prompt):
    ...
    if model:
        try:
            with st.spinner("🤖 AI 交易教練正在分析中 (Gemini)..."):
                response = model.generate_content(prompt)
                return response.text
        except Exception as e:
            st.warning(f"Gemini 失敗，嘗試備援: {e}")
    if BACKUP_API_KEY and BACKUP_BASE_URL:
        try:
            with st.spinner("⚠️ 切換至備援 AI 分析中..."):
...
def clean_strategy(s):
    s_str = str(s).strip()
    if "PULLBACK" in s_str.upper():
        return "Pullback"
    if "BREAKOUT" in s_str.upper() or "BREAK OUT" in s_str.upper():
        return "Breakout"
    return s_str
...
def calculate_position_percentage(active_pos, symbol, live_prices, current_equity):
    ...
    if not current_price:
        return 0, 0
    position_value_hkd = get_hkd_value(symbol, current_price * qty)
...
def generate_llm_export_data(df, stats_summary, active_pos, live_prices, current_equity):
    ...
    if active_pos:
        for s, d in active_pos.items():
            ...
            pos_value_hkd, pos_pct = calculate_position_percentage(active_pos, s, live_prices, current_equity)
    ...
    try:
        ...
        for col in ["market_condition", "mistake_tag", "img", "trade_id"]:
            if col not in df.columns:
                df[col] = "N/A" if col != "img" else None
...
def update_transaction(row_id, updates):
    try:
        client = get_supabase()
        client.table("trades").update(updates).eq("id", row_id).execute()
        st.success("已更新")
    except Exception as e:
...
def delete_transaction(row_id):
    try:
        client = get_supabase()
        client.table("trades").delete().eq("id", row_id).execute()
    except Exception as e:
...
def calculate_portfolio(df):
    empty = {
        'active_pos': {}, 'realized_pnl_hkd': 0, 'completed_trades': pd.DataFrame(),
        'equity_curve': pd.DataFrame(), 'exp_hkd': 0, 'exp_r': 0, 'avg_dur': 0,
        'pl_ratio': 0, 'max_drawdown': 0, 'max_wins': 0, 'max_losses': 0, 'avg_risk': 0,
    }
    if df.empty:
        return empty
    ...
    for _, row in df.iterrows():
        sym = row['symbol']  # already cleaned by load_data
        ...
        if pd.isna(t_id) or str(t_id) == "N/A":
            t_id = f"LEGACY_{sym}"

        action_upper = action.upper()
        is_buy = "買入" in action or "BUY" in action_upper
        is_sell = "賣出" in action or "SELL" in action_upper
        ...
        if is_buy:
            ...
            curr['qty'] += qty
            if curr['qty'] > 0:
                curr['avg_price'] = total_cost_base / curr['qty']
            if sl > 0:
                curr['last_sl'] = sl
                ...
                try:
                    duration = float(
                        (datetime.strptime(date_str, '%Y-%m-%d') -
                         datetime.strptime(cycle_data['start_date'], '%Y-%m-%d')).days
                    )
                except Exception:
                    duration = 0
    ...
    max_wins, max_losses, avg_risk_per_trade = 0, 0, 0
    ...
    if not comp_df.empty:
        ...
        if equity_curve:
            ...
            max_drawdown = (eq_series - rolling_max).min()
        ...
        base_capital = max(INITIAL_CAPITAL + total_realized_pnl_hkd, INITIAL_CAPITAL)
    ...
    return {
        'active_pos': active_output,
        'realized_pnl_hkd': total_realized_pnl_hkd,
        'completed_trades': comp_df,
        ...
        'avg_risk': avg_risk_per_trade,
    }

@st.cache_data(ttl=60)
def get_live_prices(symbols_tuple):
...
# --- Load data & compute portfolio once ---

df = load_data()
portfolio = calculate_portfolio(df)

active_pos = portfolio['active_pos']
realized_pnl_total_hkd = portfolio['realized_pnl_hkd']
completed_trades_df = portfolio['completed_trades']
equity_df = portfolio['equity_curve']
exp_val = portfolio['exp_hkd']
exp_r_val = portfolio['exp_r']
avg_dur_val = portfolio['avg_dur']
pl_ratio_val = portfolio['pl_ratio']
mdd_val = portfolio['max_drawdown']
max_wins_val = portfolio['max_wins']
max_losses_val = portfolio['max_losses']
avg_risk_val = portfolio['avg_risk']

current_equity = INITIAL_CAPITAL + realized_pnl_total_hkd
live_prices = get_live_prices(tuple(active_pos.keys()))
...
def update_pos_pct():
    try:
        ...
        value_hkd = get_hkd_value(symbol_val, st.session_state.sb_price * st.session_state.sb_qty)
        st.session_state.sb_pos_pct = (value_hkd / current_equity) * 100
    except Exception:
        pass
...
def update_qty():
    try:
        ...
        if st.session_state.sb_price > 0:
            val_hkd = current_equity * (st.session_state.sb_pos_pct / 100)
            multiplier = 1.0 if ".HK" in symbol_val else USD_HKD_RATE
            st.session_state.sb_qty = (val_hkd / multiplier) / st.session_state.sb_price
    except Exception:
        pass
...
def update_risk_pct():
    try:
        ...
        st.session_state.sb_risk_pct = (risk_amt_hkd / current_equity) * 100
    except Exception:
        pass
...
def update_sl():
    try:
        ...
        if st.session_state.sb_qty > 0:
            risk_amt_hkd = current_equity * (st.session_state.sb_risk_pct / 100)
            multiplier = 1.0 if ".HK" in symbol_val else USD_HKD_RATE
            dist = (risk_amt_hkd / multiplier) / st.session_state.sb_qty
...
def handle_save_transaction(active_pos_data):
    ...
    assigned_tid = "N/A"
    if not is_sell:
        assigned_tid = active_pos_data[s_in]['trade_id'] if s_in in active_pos_data else int(time.time())
    else:
    ...
    if st.session_state.get('sb_img') is not None:
        ...
        st.warning("⚠️ 圖片已記錄檔名，但尚未上傳至儲存空間，請手動上傳。")
    ...
    for key in ['sb_price', 'sb_qty', 'sb_sl', 'sb_pos_pct', 'sb_risk_pct']:
...
with st.sidebar:
    ...
    st.date_input("日期", value=datetime.now(), key='sb_date')
    st.text_input("代號 (Ticker)", key='sb_symbol')
    st.toggle("Buy 🟢 / Sell 🔴", value=False, key='sb_is_sell', on_change=update_sl)

    col1, col2 = st.columns(2)
    col1.number_input("股數 (Qty)", min_value=0.0, step=100.0, key='sb_qty', on_change=update_all_metrics)
    col2.number_input("成交價格 (price)", min_value=0.0, step=0.05, key='sb_price', on_change=update_all_metrics)

    st.number_input("停損價格 (Stop Loss)", min_value=0.0, step=0.05, key='sb_sl', on_change=update_risk_pct)

    st.divider()
    st.number_input("該筆交易佔整體倉位的 %", min_value=0.0, max_value=100.0, step=1.0, key='sb_pos_pct', on_change=update_qty)
    st.number_input("停損幅度佔整體倉位的 %", min_value=0.0, max_value=100.0, step=0.1, key='sb_risk_pct', on_change=update_sl)
    st.divider()

    st.selectbox("市場環境", ["Trending Up", "Trending Down", "Range/Choppy", "High Volatility", "N/A"], key='sb_mkt')
    st.selectbox("錯誤標籤", ["None", "Fomo", "Revenge Trade", "Fat Finger", "Late Entry", "Moved Stop"], key='sb_mistake')
    st.selectbox("策略 (Strategy)", ["Pullback", "Breakout", "➕ 新增..."], key='sb_strat')
    if st.session_state.get('sb_strat') == "➕ 新增...":
        st.text_input("輸入新策略名稱", key='sb_strat_new')

    st.select_slider("心理狀態", options=["恐慌", "猶豫", "平靜", "自信", "衝動"], value="平靜", key='sb_emo')
    st.text_area("決策筆記", key='sb_note')
    st.file_uploader("📸 上傳圖表截圖", type=['png', 'jpg', 'jpeg'], key='sb_img')

    st.button("儲存執行紀錄", type="primary", use_container_width=True,
              on_click=handle_save_transaction, args=(active_pos,))
...
with t1:
    ...
    if not filtered_comp.empty:
        ...
        if "今年" in time_frame:
            ...
            mask = filtered_comp['Exit_DT'] >= (today - timedelta(days=today.weekday()))
        elif "3個月" in time_frame:
            mask = filtered_comp['Exit_DT'] >= (today - timedelta(days=90))
        else:
    ...
    for s, d in active_pos.items():
        ...
        if curr_price and d['last_sl'] > 0:
            potential_stop_loss_impact += get_hkd_value(s, (curr_price - d['last_sl']) * d['qty'])
    ...
    k1.metric("若全體止損回撤", mask_val(potential_stop_loss_impact, "-${:,.0f}"), delta_color="inverse",
              help="若所有當前持倉立刻打到止損價，帳戶市值將減少的金額")
    if max_wins_val > 0:
    ...
    k4.metric("目前帳戶預估", mask_val(current_equity, "${:,.0f}"))
    ...
    if st.button("生成本期 AI 檢討報告"):
        if filtered_comp.empty:
            ...
            prompt = generate_llm_export_data(filtered_comp, export_stats, active_pos, live_prices, current_equity)
    ...
    if not filtered_comp.empty:
        ...
        display_trades['原始損益'] = display_trades.apply(
            lambda x: ("****" if private_mode else
                       f"{get_currency_symbol(x['symbol'])} {x['PnL_Raw']:,.2f}"), axis=1
        )
        ...
        with r1:
            st.markdown("##### 🟢 Top 獲利")
            st.dataframe(display_trades.sort_values("PnL_HKD", ascending=False).head(5)[['出場日期', '代號', '原始損益', 'HKD 損益', 'R 乘數']], hide_index=True, use_container_width=True)
        with r2:
            st.markdown("##### 🔴 Top 虧損")
            st.dataframe(display_trades.sort_values("PnL_HKD", ascending=True).head(5)[['出場日期', '代號', '原始損益', 'HKD 損益', 'R 乘數']], hide_index=True, use_container_width=True)
...
with t2:
    ...
    if active_pos:
        ...
        for s, d in active_pos.items():
            ...
            init_risk_hkd = get_hkd_value(s, abs(entry_p - entry_sl) * qty) if entry_sl > 0 else 0
            curr_risk_hkd = get_hkd_value(s, (now - last_sl) * qty) if (now and last_sl > 0) else 0
        ...
        st.dataframe(
            pos_df,
            column_config={
                "報酬%": st.column_config.ProgressColumn("報酬%", format="%.2f%%", min_value=-20, max_value=20),
                "佔整體帳戶%": st.column_config.ProgressColumn("佔整體帳戶%", format="%.2f%%", min_value=0, max_value=100),
            },
            hide_index=True,
        ...
        cols = st.columns(min(len(active_pos), 4))
        for idx, (symbol, pos_data) in enumerate(active_pos.items()):
            with cols[idx % len(cols)]:
                ...
                if st.button("❌ 止損平倉", key=f"sl_btn_{symbol}", type="secondary"):
                    close_position_at_stop_loss(symbol, active_pos)

        st.divider()
        total_pos_pct = (total_position_value_hkd / current_equity) * 100 if current_equity > 0 else 0
        col_s1, col_s2, col_s3 = st.columns(3)
        col_s1.metric("總持倉市值 (HKD)", f"${total_position_value_hkd:,.0f}")
        col_s2.metric("總倉位佔比", f"{total_pos_pct:.2f}%")
        col_s3.metric("帳戶現金", f"${current_equity - total_position_value_hkd:,.0f}")
...
with t3:
    st.subheader("⏪ 交易重播")
    if not df.empty:
        target = st.selectbox("選擇交易", df.index,
                              format_func=lambda x: f"[{df.iloc[x]['date']}] {df.iloc[x]['symbol']}")
        row = df.iloc[target]
        data = yf.download(
            row['symbol'],
            start=(pd.to_datetime(row['date']) - timedelta(days=20)).strftime('%Y-%m-%d'),
            progress=False
        )
        if not data.empty:
            ...
            fig.add_trace(go.Scatter(
                x=[pd.to_datetime(row['date'])], y=[row['price']],
                mode='markers+text',
                marker=dict(size=15, color='orange', symbol='star'),
                text=["執行"], textposition="top center"
            ))
        ...
        if st.button("🤖 AI 單筆深度診斷"):
            prompt = (f"請檢討這筆交易：代號 {row['symbol']}, 進場 {row['price']}, "
                      f"策略 {row.get('strategy', 'N/A')}, 情緒 {row.get('emotion', 'N/A')}, "
                      f"錯誤 {row.get('mistake_tag', 'N/A')}。請評估其進場合理性。")
            st.markdown(get_ai_response(prompt))
...
with t4:
    ...
    if not completed_trades_df.empty:
        ...
        mistake_impact = completed_trades_df.groupby('mistake_tag').agg(
            ...
            次數=('PnL_HKD', 'count'),
            平均R=('Trade_R', 'mean')
        ).reset_index().rename(columns={'mistake_tag': '錯誤類型'})
        mistake_impact['總虧損'] = mistake_impact['總虧損'].round(0)
        mistake_impact['平均R'] = mistake_impact['平均R'].round(2)

        c_mis1, c_mis2 = st.columns([1, 2])
        with c_mis1:
            st.dataframe(mistake_impact.sort_values('總虧損'), hide_index=True, use_container_width=True)
        with c_mis2:
            st.plotly_chart(
                px.bar(mistake_impact, x='錯誤類型', y='總虧損', color='總虧損',
                       color_continuous_scale='RdYlGn', title="哪種錯誤最燒錢？"),
                use_container_width=True
            )
        ...
        dur_stats = temp_df.groupby('Duration_Bin', observed=True)['PnL_HKD'].agg(
            總損益='sum', 平均損益='mean', 次數='count'
        ).reset_index().rename(columns={'Duration_Bin': '持有週期'})
        dur_stats['總損益'] = dur_stats['總損益'].round(0)
        dur_stats['平均損益'] = dur_stats['平均損益'].round(0)
        st.plotly_chart(
            px.bar(dur_stats, x='持有週期', y='總損益', color='總損益',
                   title="不同週期的獲利表現", color_continuous_scale='RdYlGn'),
            use_container_width=True
        )
    ...
    if not df.empty:
        ...
        display_df = hist_df[available_cols].rename(columns={
            "date": "日期", "symbol": "代號", "action": "買賣",
            "trade_id": "Trade_ID", "price": "價格", "quantity": "股數",
            "stop_loss": "停損", "emotion": "情緒", "mistake_tag": "錯誤標籤", "img": "截圖"
        })
...
with t5:
    ...
    if not df.empty:
        export_stats = {
            ...
            "win_rate": (f"{len(completed_trades_df[completed_trades_df['PnL_HKD'] > 0]) / len(completed_trades_df) * 100:.1f}%"
                         if not completed_trades_df.empty else "N/A"),
            "pf": f"{pl_ratio_val:.2f}",
            "exp_r": f"{exp_r_val:.2f}R",
            "mdd": f"${mdd_val:,.0f}",
            "count": len(completed_trades_df),
        }
        export_text = generate_llm_export_data(df, export_stats, active_pos, live_prices, current_equity)
        st.download_button(
    ...
    with col_u1:
        ...
        if uploaded_file and st.button("🚀 開始匯入"):
            try:
                ...
                bulk_insert_data(new_data.to_dict(orient="records"))
                st.cache_data.clear()
                st.rerun()
            except Exception as e:
                st.error(f"匯入失敗: {e}")
    ...
    if not df.empty:
        ...
        selected_idx = st.selectbox(
            "選擇紀錄進行編輯", df.index,
            format_func=lambda x: f"[{df.loc[x, 'date']}] {df.loc[x, 'symbol']} ({df.loc[x, 'action']})"
        )
        ...
        if b1.button("💾 儲存修改", use_container_width=True):
            row_id = t_edit.get('id')
            if row_id:
                update_transaction(int(row_id), {"price": n_p, "quantity": n_q, "stop_loss": n_sl})
                st.cache_data.clear()
                st.rerun()
            else:
                st.error("找不到記錄 ID，無法更新")
